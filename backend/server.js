...
app.post("/lancamentos", verificarPermissao(PERM_LANCAMENTOS), async function (req, res) {
  try {
    ...
    const novoLancamento = {
      // id é IDENTITY no banco (GENERATED BY DEFAULT) — deixa o Postgres
      // gerar de forma atômica. Gerar aqui com Date.now() causava colisão
      // quando dois inserts caíam no mesmo milissegundo (duplo clique,
      // retry de rede, dois processos). Auditoria C1 — 01/09/2026.
      ...dadosPreparados,
      status,
...
app.put("/contas-pagar/:id/pagar", verificarPermissao(PERM_CONTAS_PAGAR), async function (req, res) {
  try {
    ...
    const novaDespesa = {
      // id gerado pelo banco (IDENTITY) — ver auditoria C1 (01/09/2026).
      tipo: "despesa",
...
async function importarVendasSaiposComoLancamentos(loja, dataStr) {
  ...
  for (const grupo of Object.values(grupos)) {
    ...
    if (existentes && existentes[0]) {
      ...
      // id gerado pelo banco (IDENTITY) — ver auditoria C1 (01/09/2026). A
      // idempotência aqui continua sendo o índice UNIQUE em chave_importacao
      // + o catch do 23505 logo abaixo.
      ...
      res.json({
        ...
        aviso_soma_nao_bate:
          avisoEsperado || avisoEmCaixa
            ...
            : null,
        // Lista crua das retiradas lidas (com data/hora, descrição, o
        // campo "Conta:" — ex. "Cofre" — e valor). Usada pela conferência
        // do dinheiro pra saber quanto a Saipos marcou como Cofre.
        retiradas_frente_caixa: retiradasLidas.map((r) => ({
          data_hora: r?.data_hora ?? null,
          descricao: r?.descricao ?? null,
          conta: r?.conta ?? null,
          valor:
            r?.valor != null ? Math.abs(Number(r.valor)) || 0 : 0,
        })),
    ...
    try {
      ...
      const retiradasCaixa =
        ...
        Number.isFinite(Number(req.body.retiradas_caixa))
          ...
          : null;
      // Lista das retiradas lidas da foto (com o campo "Conta:", ex.
      // "Cofre") — pra conferência do dinheiro saber quanto foi pro Cofre
      // segundo a própria Saipos.
      const retiradasDetalhe = Array.isArray(req.body?.retiradas_detalhe)
        ? req.body.retiradas_detalhe.slice(0, 100)
        : null;
